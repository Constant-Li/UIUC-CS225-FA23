                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    // if((single->freq < merge->freq) && (single->freq == merge->freq)){
    //     singleQueue.pop();
    //     return single;
    // }else{
    //     mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(!((singleQueue.size() == 1 && mergeQueue.empty()) || (mergeQueue.size() == 1 && singleQueue.empty()))){
        TreeNode * left = removeSmallest(singleQueue, mergeQueue);
        TreeNode * right = removeSmallest(singleQueue, mergeQueue);
        int fre = left->freq.getFrequency() + right->freq.getFrequency();
        TreeNode * internal = new TreeNode(fre);
        internal->left = left;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    while(bfile.hasBits()){
        if(bfile.getNextBit() == 1){
            // get nextbyte's data
            Frequency next = Frequency(bfile.getNextByte(), 0);
