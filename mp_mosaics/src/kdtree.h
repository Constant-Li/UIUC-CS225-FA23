...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* buildkdtree(vector<Point<Dim>>& list, int left, int right, int dim);

    // helper function for copy constructor
    void copy(KDTreeNode* current, KDTreeNode* other);

    // helper function to delete
    void Delete(KDTreeNode* Node);

    void inorder(KDTreeNode* root, vector<Point<Dim>>& result) const;
};
...
RandIter select(RandIter begin, RandIter end, RandIter k, Comparator cmp);


/**
  * Reagrrange the itr so that right is smaller than pivot and left is larger.
  */
template <typename RandIter, typename Comparator>
