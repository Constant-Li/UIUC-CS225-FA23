...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    for(int i = 0; i < (int)size; i++){
        if(table[(hash1 + i * hash2) % size] == nullptr){
            std::pair<K, V> *add = new std::pair<K, V>(key, value);
            table[(hash1 + i * hash2) % size] = add;
            should_probe[(hash1 + i * hash2) % size] = true;
            break;
        }
    }
    elems = elems + 1;
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    while(should_probe[hash1] == true){
        if((table[hash1] != nullptr) && ((table[hash1])->first == key)){
            return hash1;
        }
