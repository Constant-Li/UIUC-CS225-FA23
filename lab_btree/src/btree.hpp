...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    int first_larger_idx = insertion_idx(subroot->elements, key);
    ...
    std::vector<DataPair> nodes = subroot->elements;

    if(first_larger_idx < (int)nodes.size() && key == nodes[first_larger_idx].key){
        return nodes[first_larger_idx].value;
    }
    if(subroot->is_leaf){
        return V();
    }else{
        BTreeNode* next = subroot->children[first_larger_idx];
